...
/** Body for POST /api/fotografos/create (auth user id from server only) */
export const fotografoCreateBodySchema = z.object({
  ...
  chavePix: z.string().min(1, "Chave Pix é obrigatória").max(255),
});

/** Body for POST /api/fotografos/onboarding (update existing fotografo) */
