...
export const photographerProfileSchema = z.object({
  username: z.string().min(3).max(30).regex(/^[a-z0-9_-]+$/i, "Username must contain only letters, numbers, underscores, and dashes"),
...
export const uploadMetadataSchema = z.object({
  ...
  orientacao: z.enum(["HORIZONTAL", "VERTICAL", "QUADRADO", "PANORAMICA"]).optional(),
});

export const uploadRequestSchema = z.object({
  filename: z.string().min(1, "Filename is required"),
  contentType: z.string().regex(/^image\/(jpeg|png|webp|avif)$/, "Invalid image type"),
  folder: z.string().regex(/^[a-z0-9-_/]+$/i, "Invalid folder name").optional(),
});
