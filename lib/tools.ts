...
const getUserFromDB = (userId: string) => {
...
export const availableTools = {
  get_user_profile: ({ userId }: { userId: string }) => {
  ...
  list_directory: async ({ dirPath }: { dirPath: string }) => {
    try {
      const targetDir =
        dirPath === "." ? process.cwd() : path.resolve(process.cwd(), dirPath);
