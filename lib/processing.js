...
export async function processUploadedImage(s3Key, photoId) {
  ...
  try {
    ...
    // 4. Apply Tiled Text Watermark
    try {
      // Calculate proportional watermark size
      const resizedMeta = await sharp(resizedBuffer).metadata();
      const minDimension = Math.min(resizedMeta.width, resizedMeta.height);

      // Calculate tile size and font size
      // Tile size approx 30% of min dimension for good density
      const tileSize = Math.max(150, Math.round(minDimension * 0.3));
      const fontSize = Math.round(tileSize * 0.25); // Font size relative to tile

      console.log(
        `[Processing] Generating text watermark. Tile: ${tileSize}px, Font: ${fontSize}px`,
      );

      // Create SVG with rotated text
      const svgText = `
        <svg width="${tileSize}" height="${tileSize}" xmlns="http://www.w3.org/2000/svg">
          ...
          <text x="50%" y="50%" class="text" transform="rotate(-45, ${tileSize / 2}, ${tileSize / 2})">GT Clicks</text>
        </svg>
      `;
