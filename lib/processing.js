...
/**
 * Uploads a buffer to S3 com Cache-Control para CDN/browser.
 */
...
export async function processUploadedImage(s3Key, photoId) {
  ...
  try {
    ...
    // 3. Optimize for Preview (1200px suficiente para cards/grid; reduz LCP)
    ...
    // Garantir formato JPEG antes da marca d'água (quality 82 = bom tamanho/qualidade)
...
export async function generateCoverImage(s3Key) {
  try {
    ...
    // 3. Resize (Max 1200px - capas em cards/grid não precisam de mais)
    const MAX_DIMENSION = 1200;
    let pipeline = image.rotate();
    ...
    // 4. Output as JPEG (Clean, No Watermark - quality 82 para tamanho menor)
