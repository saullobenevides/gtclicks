...
export async function processUploadedImage(s3Key, photoId) {
  ...
  try {
    ...
    console.log(
      `[Processing] Downloaded original (${originalBuffer.length} bytes)`
    );
    ...
    try {
      ...
      // Resize watermark to 200x200 to match previous CSS pattern size
      ...
      return {
        ...
        previewKey,
        indexingStatus:
          indexingResult.FaceRecords?.length > 0 ? "INDEXED" : "FAILED",
      ...
      // Fallback: upload resizedBuffer without watermark
      const originalName = s3Key.split("/").pop();
      const previewKey = `previews/${originalName}`;
      const previewUrl = await uploadToS3(
        previewKey,
