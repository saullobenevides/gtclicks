...
export default function LocationSelector({ selectedState, selectedCity, onStateChange, onCityChange }) {
  ...
  useEffect(() => {
    fetch('https://servicodados.ibge.gov.br/api/v1/localidades/estados?orderBy=nome')
      .then(res => res.json())
      .then(data => {
        const formattedStates = data.map(uf => ({ id: uf.id, sigla: uf.sigla, nome: uf.nome }));
  ...
  useEffect(() => {
    if (selectedState) {
      ...
      fetch(`https://servicodados.ibge.gov.br/api/v1/localidades/estados/${selectedState}/municipios`)
        .then(res => res.json())
        .then(data => {
            const formattedCities = data.map(city => ({ id: city.id, nome: city.nome }));
    ...
    <>
      <div className="space-y-1.5 flex flex-col">
        ...
        <Popover open={openState} onOpenChange={setOpenState}>
          ...
          <PopoverContent className="w-[300px] p-0 bg-zinc-950 text-white border-zinc-800">
            <Command>
                ...
                <CommandGroup>
                  {states.map((state) => (
                    <CommandItem
                      ...
                      onSelect={() => {
                        onStateChange(state.sigla === selectedState ? "" : state.sigla);
                        onCityChange('');
                        setOpenState(false);
                      }}
                    >
                  ...
                  {cities.map((city) => (
                    <CommandItem
                      ...
                      onSelect={(currentValue) => {
                        onCityChange(currentValue === selectedCity ? "" : currentValue)
                        setOpenCity(false)
                      }}
                    >
                      <Check
