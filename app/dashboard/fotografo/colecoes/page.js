            ...
            <TableBody>
                {colecoes.length === 0 ? (
                    ...
                    colecoes.map((colecao) => (
                        <TableRow key={colecao.id} className="border-white/10 hover:bg-white/5">
                            <TableCell className="font-medium">
                                <div className="flex flex-col">
                                    <span className="text-base text-white">{colecao.nome}</span>
                                    {colecao.descricao && (
                            ...
                            <TableCell>
                                <Badge variant={colecao.status === 'PUBLICADA' ? 'default' : 'secondary'} className={colecao.status === 'PUBLICADA' ? 'bg-green-500/10 text-green-500 hover:bg-green-500/20' : ''}>
                            ...
                            <TableCell className="text-right">
                                <div className="flex justify-end gap-2">
                                    <Button asChild variant="ghost" size="icon" className="h-8 w-8 text-muted-foreground hover:text-white">
                                        <Link href={`/dashboard/fotografo/colecoes/${colecao.id}/editar`} title="Gerenciar">
                                            <Edit className="h-4 w-4" />
                                        </Link>
                                    </Button>
                                    {colecao.status === 'PUBLICADA' && (
                                        <Button asChild variant="ghost" size="icon" className="h-8 w-8 text-muted-foreground hover:text-white">
                                            <Link href={`/colecoes/${colecao.slug}`} target="_blank" title="Ver Página Pública">
