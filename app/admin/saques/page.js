...
export default function AdminSaquesPage() {
  ...
  const handleProcessar = async (saqueId, action) => {
    ...
    try {
      ...
      if (response.ok) {
        setMessage({ type: 'success', text: `Saque ${action === 'aprovar' ? 'aprovado' : 'cancelado'} com sucesso!` });
        fetchSaques();
      } else {
        const data = await response.json();
        setMessage({ type: 'error', text: data.error || 'Erro ao processar saque.' });
      }
    } catch (error) {
  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      {loading ? <SaquesSkeleton /> : (
        <>
            <div className='space-y-6'>
                <h2 className="text-2xl font-bold">Pendentes ({pendentes.length})</h2>
                {pendentes.length === 0 ? (
                    ...
                    {pendentes.map((saque) => (
                        ...
                        <CardContent className="space-y-4">
                            ...
                            <div>
                                <h4 className="text-sm font-semibold text-muted-foreground">Chave PIX</h4>
                        ...
                        <CardFooter className="gap-4">
                            <Button
                                ...
                                className="flex-1"
                            >
                                {processing === saque.id && <Loader2 className="mr-2 h-4 w-4 animate-spin"/>}
                                Aprovar
                            </Button>
                            <Button
            ...
            <div className='space-y-6'>
                <h2 className="text-2xl font-bold">Processados ({processados.length})</h2>
                <Card>
                <Table>
                    <TableHeader>
                    <TableRow>
                    ...
                    {processados.length === 0 ? (
                        ...
                        <TableRow key={saque.id}>
                            ...
                            <TableCell>R$ {Number(saque.valor).toFixed(2)}</TableCell>
                            <TableCell>
                            {new Date(saque.createdAt).toLocaleDateString('pt-BR')}
                            </TableCell>
                            <TableCell>
                            <Badge variant={saque.status === 'APROVADO' ? 'default' : 'outline'}>
                                {saque.status}
                            </Badge>
                            </TableCell>
                        </TableRow>
