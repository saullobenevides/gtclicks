...
export async function PUT(request) {
  try {
    ...
    // Authorization: Ideally verify if user is PHOTOGRAPHER, but allow if they are just converting?
    // For update, they should likely be a photographer.
    // However, if they are converting from client, they might not have the role yet in some flows?
    // But this involves "prisma.fotografo.update", so the record MUST exist.
    // If usage is constrained to photographers, checking existence is enough.

    const body = await request.json();
    const { bio, telefone, cidade, estado, instagram, chavePix } = body;

    // Use authenticated ID, IGNORE body.userId for security
