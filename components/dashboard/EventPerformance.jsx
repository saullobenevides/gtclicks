...
import { Eye, DollarSign, Users, ShoppingBag, TrendingUp, AlertCircle } from "lucide-react";
  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="text-muted-foreground">Relatório de Performance do Evento</p>
      </div>
        ...
        <Card className="col-span-4">
          ...
          <CardContent>
            <div className="space-y-4">
                {rankings.bestSellers.map((photo, i) => (
                    <div key={photo.id} className="flex items-center gap-4">
                        <div className="relative h-12 w-12 rounded overflow-hidden">
                            <Image src={photo.thumb} alt={photo.title} fill className="object-cover" />
                        </div>
                        <div className="flex-1">
                            <p className="font-medium text-sm line-clamp-1">{photo.title || `Foto ${photo.number || 'Sem número'}`}</p>
                            <p className="text-xs text-muted-foreground">{photo.views} views</p>
                        </div>
                ...
                {rankings.bestSellers.length === 0 && <p className="text-muted-foreground text-sm">Nenhuma venda ainda.</p>}
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="col-span-3">
          ...
          <CardContent>
            <p className="text-xs text-muted-foreground mb-4">
                Fotos com muitas visualizações mas 0 vendas. Considere baixar o preço ou usar em destaque.
            </p>
            <div className="space-y-4">
                {rankings.windowShoppers.map((photo, i) => (
                    <div key={photo.id} className="flex items-center gap-4 opacity-80"> 
                        <div className="relative h-10 w-10 rounded overflow-hidden grayscale">
                            <Image src={photo.thumb} alt={photo.title} fill className="object-cover" />
                        </div>
                        <div className="flex-1">
                            <p className="font-medium text-xs">#{photo.number || '---'}</p>
                        </div>
                 ...
                 {rankings.windowShoppers.length === 0 && <p className="text-muted-foreground text-sm">Nenhum dado relevante.</p>}
            </div>
          </CardContent>
        </Card>
