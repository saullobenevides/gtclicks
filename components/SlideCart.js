        ...
        <div className="flex-1 overflow-y-auto py-4">
          {items.length === 0 ? (
            ...
            <div className="space-y-4">
              {items.map((item) => (
                <div key={item.fotoId} className="flex gap-4 border-b border-white/5 pb-4 last:border-0">
                   ...
                   <div className="flex flex-1 flex-col justify-between">
                      <div className="flex justify-between gap-2">
                         <h4 className="font-medium line-clamp-2 text-sm">{item.titulo}</h4>
                         <p className="font-bold text-sm">R$ {Number(item.preco).toFixed(2)}</p>
                      </div>
                      <div className="flex items-center justify-between">
                         <span className="text-xs text-muted-foreground">{item.licenca}</span>
                         <Button 
                           variant="ghost" 
                           size="icon" 
        ...
        {items.length > 0 && (
          <SheetFooter className="flex-col gap-4 border-t border-white/10 pt-4 sm:flex-col sm:space-x-0">
             ...
             <div className="grid gap-2">
                <Button asChild size="lg" className="w-full bg-primary hover:bg-primary/90 text-white font-bold" onClick={() => setIsCartOpen(false)}>
                   <Link href="/checkout">Finalizar Compra</Link>
                </Button>
                <Button variant="outline" className="w-full border-white/10 hover:bg-white/5 text-white" onClick={() => setIsCartOpen(false)}>
