  ...
  const faqs = [
    {
      ...
      answer: "Criar uma conta e publicar suas fotos é totalmente gratuito. Cobramos apenas uma pequena taxa de serviço sobre cada venda realizada."
    },
    {
      question: "Em quanto tempo eu recebo o pagamento da foto vendida?",
      answer: "Os valores ficam disponíveis para saque 7 dias após a confirmação da venda."
    },
    {
      question: "Como eu recebo meu pagamento?",
      answer: "Os pagamentos são processados via PIX diretamente para sua conta cadastrada."
    },
    {
      question: "Minhas fotos estão protegidas?",
      answer: "Sim! Todas as fotos são exibidas com marca d'água e em baixa resolução para visualização. O arquivo original em alta resolução só é liberado após a confirmação do pagamento."
    }
  ];
          ...
          {faqs.map((faq, index) => (
            ...
            >
              <AccordionTrigger className="px-8 hover:no-underline transition-colors border-transparent text-left py-5 text-base font-bold text-white flex-row-reverse justify-end gap-4 [&>svg]:text-[#FF0000] [&>svg]:transition-transform [&>svg]:duration-300 [&>svg]:h-5 [&>svg]:w-5 [&>svg]:-rotate-90 [&[data-state=open]>svg]:!rotate-0">
                <span className="[&[data-state=open]]:text-[#FF0000]">{faq.question}</span>
              </AccordionTrigger>
